...
|------|-----------|-----------|------|
| **单元测试** | 8 | 127/127 | ✅ **全部通过** |
| 集成测试 | 多个 | 需要独立数据库 | 🔄 辅助函数已创建 |
| API 测试 | 多个 | 需要独立数据库 | 🔄 辅助函数已创建 |
| SDK 测试 | 1 | 需要独立数据库 | 🔄 辅助函数已创建 |
...
```typescript
import { createIsolatedTestDatabase, dropIsolatedTestDatabase } from '../helpers/isolated-database.js';
...
集成测试文件需要逐个更新使用独立数据库辅助函数。主要涉及的文件：
