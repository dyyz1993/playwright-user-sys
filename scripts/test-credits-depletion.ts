...
const SESSION_DURATION = 10; // 会话持续时间（分钟）
  ...
  try {
    ...
    const checkInterval = setInterval(async () => {
      try {
        ...
        console.log(`已运行 ${elapsedMinutes} 分钟，当前点数: ${updatedUser.credits}`);
    ...
    console.log(`预计会话将在 ${SESSION_DURATION} 分钟后结束`);
    console.log('保持浏览器打开，等待点数耗尽...');

    // 设置错误处理
    browser.on('disconnected', () => {
      console.log('浏览器已断开连接，可能是由于点数耗尽');
      clearInterval(checkInterval);
      process.exit(0);
    });
    ...
    setTimeout(() => {
      ...
      clearInterval(checkInterval);
      process.exit(0);
    }, SESSION_DURATION * 60 * 1000 + 60000); // 会话时间 + 1分钟的缓冲
  } catch (error) {
