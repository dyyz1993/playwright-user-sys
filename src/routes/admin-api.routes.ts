...
export default async function adminApiRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/api/admin/users', {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        return reply.status(400).send({ success: false, error: '无效的查询参数: ' + error.errors.map(e => e.message).join(', ') });
      }
  ...
  // 添加新机器（管理员）
  fastify.post('/api/admin/machines', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 批量删除用户
  fastify.post('/api/admin/users/batch-delete', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 批量充值
  fastify.post('/api/admin/users/batch-recharge', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
    ...
    try {
      const adminId = request.user?.id;
      const body = request.body as { userIds: number[]; credits: number; reason?: string };

      if (!body.userIds || !Array.isArray(body.userIds) || body.userIds.length === 0) {
        return reply.status(400).send({ success: false, error: '请提供要充值的用户 ID 列表' });
      }

      const amount = parseInt(body.credits as any);
      if (isNaN(amount) || amount <= 0) {
        return reply.status(400).send({ success: false, error: '无效的点数金额' });
      }
  ...
  // 批量结束会话
  fastify.post('/api/admin/sessions/batch-release', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 获取用户操作日志
  fastify.get('/api/admin/users/:id/logs', {
    preHandler: [authenticate],
    schema: {
      params: zodToJsonSchema(idParamSchema),
      response: {
  ...
  // 批量重启机器
  fastify.post('/api/admin/machines/batch-restart', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 获取用户存储统计
  fastify.get('/api/admin/storage/stats', {
    preHandler: [authenticate],
    schema: {
      querystring: {
        type: 'object',
  ...
  // 清理用户数据
  fastify.post('/api/admin/storage/cleanup', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 清理所有旧数据
  fastify.post('/api/admin/storage/cleanup-all', {
    preHandler: [authenticate],
    schema: {
      body: {
        type: 'object',
  ...
  // 获取系统存储统计
  fastify.get('/api/admin/storage/system-stats', {
    preHandler: [authenticate],
    schema: {
      response: {
        200: zodToJsonSchema(z.object({
