import puppeteer from 'puppeteer-core';
import fetch from 'node-fetch';

// 配置
const API_KEY = 'api_key_1'; // 使用模拟的 API Key
const API_BASE_URL = 'http://localhost:3000/api';
const SESSION_URL = `${API_BASE_URL}/sessions`;

async function main() {
  try {
    console.log('开始测试流程...');

    // 步骤 1：创建会话
    console.log('步骤 1：创建会话...');
    const sessionResponse = await fetch(SESSION_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': API_KEY,
      },
      body: JSON.stringify({
        viewport: {
          width: 1280,
          height: 800,
        },
      }),
    });

    if (!sessionResponse.ok) {
      const errorText = await sessionResponse.text();
      throw new Error(`创建会话失败: ${sessionResponse.status} ${errorText}`);
    }

    const sessionData = await sessionResponse.json();
    console.log('会话创建成功:', sessionData);

    const sessionId = sessionData.data.id;
    const browserWSEndpoint = sessionData.data.browserWSEndpoint;

    // 步骤 2：连接到浏览器
    console.log('步骤 2：连接到浏览器...');
    console.log(`WebSocket 端点: ${browserWSEndpoint}`);

    // 确保 WebSocket URL 包含 apiKey
    const wsEndpoint = browserWSEndpoint.includes('apiKey')
      ? browserWSEndpoint
      : `${browserWSEndpoint}${browserWSEndpoint.includes('?') ? '&' : '?'}apiKey=${API_KEY}`;

    console.log(`完整 WebSocket 端点: ${wsEndpoint}`);

    const browser = await puppeteer.connect({
      browserWSEndpoint: wsEndpoint,
      defaultViewport: { width: 1280, height: 800 },
    });

    console.log('成功连接到浏览器');

    // 步骤 3：打开百度
    console.log('步骤 3：打开百度...');
    const page = await browser.newPage();
    await page.goto('https://www.baidu.com');
    console.log('成功打开百度');

    // 步骤 4：截图
    console.log('步骤 4：截图...');
    await page.screenshot({ path: 'baidu.png' });
    console.log('截图已保存为 baidu.png');

    // 步骤 5：在百度搜索框中输入内容
    console.log('步骤 5：在百度搜索框中输入内容...');
    await page.type('#kw', 'Playwright 自动化测试');
    await page.click('#su');
    console.log('成功搜索');

    // 等待搜索结果加载
    await page.waitForSelector('.result');

    // 再次截图
    console.log('步骤 6：搜索结果截图...');
    await page.screenshot({ path: 'baidu-search.png' });
    console.log('搜索结果截图已保存为 baidu-search.png');

    // 等待 5 秒
    console.log('等待 5 秒...');
    await new Promise(resolve => setTimeout(resolve, 5000));

    // 步骤 7：关闭浏览器
    console.log('步骤 7：关闭浏览器...');
    await browser.close();
    console.log('浏览器已关闭');

    // 步骤 8：释放会话
    console.log('步骤 8：释放会话...');
    const releaseResponse = await fetch(`${SESSION_URL}/${sessionId}/release`, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
      },
    });

    if (!releaseResponse.ok) {
      const errorText = await releaseResponse.text();
      throw new Error(`释放会话失败: ${releaseResponse.status} ${errorText}`);
    }

    const releaseData = await releaseResponse.json();
    console.log('会话释放成功:', releaseData);

    console.log('测试流程完成');
  } catch (error) {
    console.error('测试流程失败:', error);
    process.exit(1);
  }
}

main();
